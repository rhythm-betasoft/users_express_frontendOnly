<template>
  <v-container>
    ...
    <div class="text-center mb-4">
      <v-btn v-if="store.user?.role === 'admin'" color="success" @click="dialog = true">
        Create Announcement
      </v-btn>
    </div>
    ...
    <v-row>
      <v-col cols="12" v-for="announcement in announcements" :key="announcement.id">
        <v-card outlined class="mb-4">
          ...
          <v-card-actions>
            <v-btn
              v-for="type in Object.keys(emojiMap)"
              :key="type"
              variant="text"
              @click="react(announcement, type)"
            >
              {{ emojiMap[type] }} {{ countReactions(announcement, type) }}
            </v-btn>
          </v-card-actions>
  ...
  methods: {
    ...
    async loadComments(announcementId) {
      try {
        ...
        if (announcement) announcement.comments = res.data
      } catch (error) {
    ...
    async loadReactions(announcementId) {
      try {
        const res = await api.get(`/reaction/${announcementId}`)
        const announcement = this.announcements.find(a => a.id === announcementId)
        if (announcement) announcement.reactions = res.data
      } catch (error) {
        console.error("Error loading reactions:", error)
      }
    },
    async react(announcement, type) {
      try {
        ...
        await this.loadReactions(announcement.id)
      } catch (error) {
        console.error("Error reacting:", error)
      }
    },
    countReactions(announcement, type) {
      return announcement.reactions?.filter(r => r.type === type).length || 0
    }
  }
}
</script>
